...
// ✅ on étend avec participantsEmails (stocké dans sessionStorage)
type AiSummary = {
    titre: string;
    date: string;
    heure: string;
    participants: string[];
    participantsEmails?: string[]; // ✅ NEW
...
export default function MeetingSummaryPage() {
    ...
    // ✅ Génère le PDF et déclenche un téléchargement
    ...
    // ✅ Génère le PDF + envoie via API (mail.send côté backend)
    async function sendPdfToParticipants() {
        ...
        try {
            // 1) Générer le PDF (on récupère les bytes)
